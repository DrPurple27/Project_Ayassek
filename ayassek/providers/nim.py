...
class NIMProvider(BaseProvider):
    ...
    def __init__(self, base_url: str, api_key: str | None = None):
        super().__init__(base_url or "https://ai.api.nvidia.com/v1", api_key)
        self._logger = get_logger("nim_provider")
    ...
    async def list_models(self) -> list[ModelInfo]:
        try:
            ...
            self._logger.warning("Failed to list NIM models: %s", e)
            return []
    ...
    async def _non_stream_chat(self, payload: dict[str, Any]) -> ChatResponse:
        try:
            ...
            self._logger.error("NIM API error: %s", e)
            raise
        except httpx.RequestError as e:
            self._logger.error("NIM request failed: %s", e)
            raise
        data = resp.json()
        if not data.get("choices"):
            self._logger.error("NIM response missing choices: %s", data)
            raise RuntimeError("No choices in NIM response")
    ...
    async def _stream_chat(self, payload: dict[str, Any]):
        try:
            ...
            self._logger.error("NIM streaming error: %s", e)
            raise
        except httpx.RequestError as e:
            self._logger.error("NIM streaming request failed: %s", e)
            raise
