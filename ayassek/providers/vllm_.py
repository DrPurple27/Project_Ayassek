...
class VLLMProvider(BaseProvider):
    ...
    def __init__(self, base_url: str, api_key: str | None = None):
        super().__init__(base_url or "http://localhost:8000/v1", api_key)
        self._logger = get_logger("vllm_provider")
    ...
    async def list_models(self) -> list[ModelInfo]:
        try:
            ...
            self._logger.warning("Failed to list vLLM models: %s", e)
            return []
    ...
    async def _non_stream_chat(self, payload: dict[str, Any]) -> ChatResponse:
        try:
            ...
            self._logger.error("vLLM API error: %s", e)
            raise
        except httpx.RequestError as e:
            self._logger.error("vLLM request failed: %s", e)
            raise
        data = resp.json()
        if not data.get("choices"):
            self._logger.error("vLLM response missing choices: %s", data)
    ...
    async def _stream_chat(self, payload: dict[str, Any]):
        try:
            ...
            self._logger.error("vLLM streaming error: %s", e)
            raise
        except httpx.RequestError as e:
            self._logger.error("vLLM streaming request failed: %s", e)
            raise
